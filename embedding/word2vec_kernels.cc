...
class ContextSkipgramWord2vecOp : public OpKernel {
 public:
  explicit ContextSkipgramWord2vecOp(OpKernelConstruction* ctx)
    ...
    for (int i = 0; i < kPrecalc; ++i) {
//    todo: get contexts
      NextExample(&precalc_examples_[i].input, &precalc_examples_[i].label, &precalc_examples_[i].context);
    }
  }
...
class FullContextSkipgramWord2vecOp : public OpKernel {
 public:
  explicit FullContextSkipgramWord2vecOp(OpKernelConstruction* ctx)
      : OpKernel(ctx), rng_(&philox_) {
    string filename;
    OP_REQUIRES_OK(ctx, ctx->GetAttr("filename", &filename));
    OP_REQUIRES_OK(ctx, ctx->GetAttr("batch_size", &batch_size_));
...
//  TODO: get context vector
  void NextExample(int32* example, int32* label, std::array<int32, 2*CONTEXT_WINDOW>* context) EXCLUSIVE_LOCKS_REQUIRED(mu_) {
    ...
    *example = sentence_[sentence_index_];
    *label = sentence_[label_pos_++];
    int32 context_start = sentence_index_ - CONTEXT_WINDOW + 1;
    int32 context_end = sentence_index_ + CONTEXT_WINDOW;
    if(context_start < 0){
        context_end += 0 - context_start;
        context_start = 0;
...
REGISTER_KERNEL_BUILDER(Name("FullContextSkipgramWord2vec").Device(DEVICE_CPU), FullContextSkipgramWord2vecOp);
